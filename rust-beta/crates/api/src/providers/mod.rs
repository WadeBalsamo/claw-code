...
/// Returns true when the user has pointed OPENAI_BASE_URL at a proxy
/// (e.g. OpenRouter, Ollama) that is not the native DashScope endpoint.
/// When true, qwen/* and kimi/* models should be routed through the proxy
/// instead of being hard-wired to DashScope.
fn openai_proxy_is_active() -> bool {
    std::env::var("OPENAI_BASE_URL")
        .ok()
        .filter(|url| !url.is_empty() && !url.contains("dashscope.aliyuncs.com"))
        .is_some()
}

/// Returns true when the user has pointed ANTHROPIC_BASE_URL at something
/// other than the default Anthropic endpoint, indicating they are proxying
/// through a local/self-hosted server (e.g. LM Studio via lmcode).
fn anthropic_proxy_is_active() -> bool {
    std::env::var("ANTHROPIC_BASE_URL")
        .ok()
        .filter(|url| {
            !url.is_empty()
                && !url.contains("api.anthropic.com")
                && !url.contains("api.anthropic.ai")
        })
        .is_some()
}
...
pub fn metadata_for_model(model: &str) -> Option<ProviderMetadata> {
    ...
    // When OPENAI_BASE_URL is set to a non-DashScope proxy (e.g. OpenRouter),
    // defer to that proxy instead so the full namespaced model ID is honoured.
    // When ANTHROPIC_BASE_URL is set to a non-Anthropic endpoint (e.g. LM
    // Studio via lmcode), route through the Anthropic env-vars via the
    // OpenAI-compatible client.
    // When neither a proxy nor DashScope credentials are available, return
    // None so detect_provider_kind() can discover the user's actual provider
    // via credential sniffing (e.g. Anthropic proxied through LM Studio).
    if canonical.starts_with("qwen/") || canonical.starts_with("qwen-") {
        if openai_proxy_is_active() {
            return Some(ProviderMetadata {
                provider: ProviderKind::OpenAi,
                auth_env: "OPENAI_API_KEY",
                base_url_env: "OPENAI_BASE_URL",
        ...
        if anthropic_proxy_is_active() && openai_compat::has_api_key("ANTHROPIC_API_KEY") {
            return Some(ProviderMetadata {
                provider: ProviderKind::OpenAi,
                auth_env: "ANTHROPIC_API_KEY",
                base_url_env: "ANTHROPIC_BASE_URL",
    ...
    // Same proxy-override logic as qwen above.
...
mod tests {
    ...
    #[test]
    fn qwen_prefix_routes_to_dashscope_when_no_proxy() {
        let _lock = env_lock();
        let _base_url = EnvVarGuard::set("OPENAI_BASE_URL", None);
        let _dashscope_key = EnvVarGuard::set("DASHSCOPE_API_KEY", Some("test-key"));

        let meta = super::metadata_for_model("qwen/qwen-max")
            .expect("qwen/ prefix must resolve to DashScope metadata when DASHSCOPE_API_KEY is set");
        ...
        let meta2 = super::metadata_for_model("qwen-plus")
            .expect("bare qwen- prefix must resolve to DashScope metadata");
        assert_eq!(meta2.auth_env, "DASHSCOPE_API_KEY");

        let kind = detect_provider_kind("qwen/qwen3-coder");
        assert_eq!(kind, ProviderKind::OpenAi, "qwen/ must win over auth-sniffer");
    }

    #[test]
    fn qwen_prefix_routes_to_openrouter_when_proxy_is_active() {
        let _lock = env_lock();
        let _base_url =
            EnvVarGuard::set("OPENAI_BASE_URL", Some("https://openrouter.ai/api/v1"));
        let _api_key = EnvVarGuard::set("OPENAI_API_KEY", Some("sk-or-test"));

        let meta = super::metadata_for_model("qwen/qwen3.6-35b-a3b")
            .expect("qwen/ prefix must resolve when proxy is active");
        assert_eq!(meta.provider, ProviderKind::OpenAi);
        ...
        // Bare qwen- prefix also defers to the proxy
        let meta2 = super::metadata_for_model("qwen-plus")
            .expect("qwen- prefix must resolve when proxy is active");
        assert_eq!(meta2.auth_env, "OPENAI_API_KEY");
    }

    #[test]
    fn kimi_prefix_routes_to_dashscope_when_no_proxy() {
        let _lock = env_lock();
        let _base_url = EnvVarGuard::set("OPENAI_BASE_URL", None);
        let _dashscope_key = EnvVarGuard::set("DASHSCOPE_API_KEY", Some("test-key"));

        let meta = super::metadata_for_model("kimi-k2.5")
            .expect("kimi-k2.5 must resolve to DashScope metadata when DASHSCOPE_API_KEY is set");
        assert_eq!(meta.auth_env, "DASHSCOPE_API_KEY");
        assert_eq!(meta.base_url_env, "DASHSCOPE_BASE_URL");
        assert!(meta.default_base_url.contains("dashscope.aliyuncs.com"));
        assert_eq!(meta.provider, ProviderKind::OpenAi);
        ...
        let meta2 = super::metadata_for_model("kimi/kimi-k2.5")
            .expect("kimi/kimi-k2.5 must resolve to DashScope metadata when DASHSCOPE_API_KEY is set");
        assert_eq!(meta2.auth_env, "DASHSCOPE_API_KEY");

        let meta3 = super::metadata_for_model("kimi-k1.5")
            .expect("kimi-k1.5 must resolve to DashScope metadata when DASHSCOPE_API_KEY is set");
        assert_eq!(meta3.auth_env, "DASHSCOPE_API_KEY");
    }

    #[test]
    fn qwen_prefix_returns_none_when_neither_proxy_nor_dashscope() {
        // When the user has qwen models from LM Studio via ANTHROPIC_BASE_URL,
        // metadata_for_model should return None so detect_provider_kind() can
        // fall through to credential sniffing and discover ANTHROPIC_API_KEY.
        let _lock = env_lock();
        ...
        assert!(
            super::metadata_for_model("qwen/qwen3-coder-30b").is_none(),
            "qwen/ prefix must return None when neither OPENAI_BASE_URL proxy nor DASHSCOPE_API_KEY is set"
        );

        assert!(
            super::metadata_for_model("qwen-plus").is_none(),
            "qwen- prefix must return None when neither OPENAI_BASE_URL proxy nor DASHSCOPE_API_KEY is set"
        );

        // verify that detect_provider_kind falls through correctly
        assert_eq!(
            detect_provider_kind("qwen/qwen3-coder-30b"),
            ProviderKind::Anthropic,
            "when no DashScope or OpenAI proxy is set, Anthropic is the fallback"
        );
    }

    #[test]
    fn kimi_prefix_routes_to_openrouter_when_proxy_is_active() {
        let _lock = env_lock();
        let _base_url =
            EnvVarGuard::set("OPENAI_BASE_URL", Some("https://openrouter.ai/api/v1"));
        let _api_key = EnvVarGuard::set("OPENAI_API_KEY", Some("sk-or-test"));

        let meta = super::metadata_for_model("kimi/kimi-k2.5")
            .expect("kimi/ prefix must resolve when proxy is active");
        assert_eq!(meta.auth_env, "OPENAI_API_KEY");
        assert_eq!(meta.base_url_env, "OPENAI_BASE_URL");

        let meta2 = super::metadata_for_model("kimi-k2.5")
            .expect("kimi- prefix must resolve when proxy is active");
        assert_eq!(meta2.auth_env, "OPENAI_API_KEY");
    }
