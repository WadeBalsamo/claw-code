...
const ANTHROPIC_PROXY_ENV_VARS: &[&str] = &["ANTHROPIC_API_KEY", "ANTHROPIC_AUTH_TOKEN"];
...
impl OpenAiCompatConfig {
    ...
    /// Generic OpenAI-compatible proxy reachable through the Anthropic
    /// env-vars (ANTHROPIC_API_KEY / ANTHROPIC_BASE_URL). Used by
    /// lmcode / LM Studio users who set ANTHROPIC_BASE_URL to point
    /// at a local server that speaks the OpenAI wire format, and
    /// want to use qwen/, kimi/, or qwen-* / kimi-* model names.
    /// The body-size limit is the least-restrictive (OpenAI's 100MB)
    /// since we don't know what proxy lies behind the endpoint.
    #[must_use]
    pub const fn anthropic_proxy() -> Self {
        Self {
            provider_name: "AnthropicProxy",
    ...
    pub fn credential_env_vars(self) -> &'static [&'static str] {
        match self.provider_name {
            ...
            "AnthropicProxy" => ANTHROPIC_PROXY_ENV_VARS,
            _ => &[],
        }
    }
}
...
/// Only strips when talking directly to the provider's native endpoint.
/// Proxies like OpenRouter require the full namespaced model ID for routing,
/// so stripping is skipped when `base_url_env` resolves to a non-default URL.
fn strip_routing_prefix<'a>(model: &'a str, config: OpenAiCompatConfig) -> &'a str {
    let is_native = std::env::var(config.base_url_env)
        .map(|url| url.is_empty() || url == config.default_base_url)
        .unwrap_or(true);
...
pub fn build_chat_completion_request(
    ...
    // Strip routing prefix (e.g., "openai/gpt-4" → "gpt-4") for the wire.
    let wire_model = strip_routing_prefix(&request.model, config);
...
mod tests {
    ...
    #[test]
    fn strip_routing_prefix_strips_kimi_provider_prefix_for_native_dashscope() {
        // Stripping only happens when routing to the native DashScope endpoint
        // (DASHSCOPE_BASE_URL not set → is_native = true).
        let cfg = super::OpenAiCompatConfig::dashscope();
        assert_eq!(super::strip_routing_prefix("kimi/kimi-k2.5", cfg), "kimi-k2.5");
        assert_eq!(super::strip_routing_prefix("kimi-k2.5", cfg), "kimi-k2.5");
        assert_eq!(super::strip_routing_prefix("kimi/kimi-k1.5", cfg), "kimi-k1.5");
    }

    #[test]
    fn strip_routing_prefix_preserves_namespaced_id_for_proxy() {
        // When OPENAI_BASE_URL points at OpenRouter the full model ID must
        // reach the proxy unchanged — it uses the namespace to route.
        use std::sync::{Mutex, OnceLock};
        static LOCK: OnceLock<Mutex<()>> = OnceLock::new();
        let _guard = LOCK
        ...
        let cfg = super::OpenAiCompatConfig::openai();
        let r1 = super::strip_routing_prefix("openai/gpt-5.5", cfg);
        let r2 = super::strip_routing_prefix("qwen/qwen3.6-35b-a3b", cfg);
        match saved {
            Some(v) => std::env::set_var("OPENAI_BASE_URL", v),
            None => std::env::remove_var("OPENAI_BASE_URL"),
        }
        assert_eq!(r1, "openai/gpt-5.5");
        assert_eq!(r2, "qwen/qwen3.6-35b-a3b");
    }
}
